...
module.exports.generateRequestListener = options => {
  ...
  return (request, response) => {
    ...
    if (_.get(q, 'pathname') === endpoint) {
      const filter = _.get(q, 'query.filter');
      ...
      return healthHandler(filter)
        ...
        .catch(err => {
          let errorCode = 400; // default to BadRequestError
          if (err.name === 'InternalError') {
            errorCode = 500;
          }

          response.writeHead(errorCode, contentType);
          const error = {
            message: err.message,
          };
    ...
    if (options.requestListener) {
      return options.requestListener(request, response);
    }
  };
};
